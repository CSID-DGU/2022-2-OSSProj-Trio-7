...
class Missile(Object):
    ...
    def __init__(self, img_path, size, power):
        ...
        super().__init__(img_path, size,
                         Default.character.value["missile"]["speed"])
        self.power = power
...
class TargetedMissile(Missile):
    ...
    def __init__(self, position, game, power):
        super().__init__(Images.weapon_target_missile.value,
                         {"x": 15, "y": 25}, power)
        self.game = game
    ...
    def find_target(self, game):
        if hasattr(game, "stage"):
            ...
            if len(game.mobList) > 0:
                ...
                if len(targets) > 0:
                    target = targets[0]
                    min = Utils.get_distance({"x": target.x, "y": target.y}, {
                    ...
                    for enemy in targets:
                        if min > Utils.get_distance({"x": enemy.x, "y": enemy.y}, {"x": game.character.x, "y": game.character.y}):
                            min = Utils.get_distance({"x": enemy.x, "y": enemy.y}, {
                                                     "x": game.character.x, "y": game.character.y})
                            target = enemy
    ...
    def find_target_pvp(self, game):
        ...
        if len(game.mobList) > 0:
            ...
            if len(targets1) > 0:
                target = targets1[0]
                min = Utils.get_distance({"x": target.x, "y": target.y}, {
                                         "x": game.character1.x, "y": game.character1.y})
        ...
        if len(game.mobList) > 0:
            ...
            if len(targets2) > 0:
                target = targets2[0]
                min = Utils.get_distance({"x": target.x, "y": target.y}, {
                                         "x": game.character2.x, "y": game.character2.y})
...
class Crosshair(Object):
    ...
    def __init__(self, target):
        super().__init__(Default.effect.value["crosshair"]["image"],
                         Default.effect.value["crosshair"]["size"], Default.effect.value["crosshair"]["velocity"])
        self.target = target
    ...
    def move(self, game):
        ...
        if self.target in game.mobList:
            self.set_XY(
                (self.target.get_pos()[0]-self.sx/2, self.target.get_pos()[1]-self.sy/2))
