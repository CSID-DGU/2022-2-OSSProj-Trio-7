...
class Character(Object):
    ...
    # org_missile_img : 기본 발사체 이미지 경로 (string)
    ...
    def __init__(self, name, img_path, velocity, missile_img, missile_size,
        ...
        self.org_missile_img = missile_img
        ...
        self.bomb_sfx = pygame.mixer.Sound(
            Default.effect.value["bomb"]["sound"])
        self.bomb_sfx.set_volume(Default.sound.value["sfx"]["volume"])
        self.gung_sfx = pygame.mixer.Sound("./Sound/Weapon/gung.wav",)
        self.gung_sfx.set_volume(Default.sound.value["sfx"]["volume"])
...
# pvp player 2 게임 실행 중 입력 키에 따라 캐릭터 이동, 발사 등의 액션을 수행
    ...
    def shoot(self):
        ...
        for num in range(1, self.fire_count+1):
            ...
            missile.x = round(
                (self.x + (num * (self.sx / div_factor))) - missile.sx / 2)
    ...
    def use_bomb(self, game):
        ...
        explosion.set_XY(
            (player_location["x"] - explosion.sx/2, player_location["y"] - explosion.sy/2))
    ...
    def use_gung(self, game):
        ...
        for i in range(4):
            i = i + 1
            gung = Gung(self.gung_img, {"x": 75, "y": 95}, self.missile_power)
            gung.change_size()
            gung.x = round((self.boundary[0] / 5) * i) - self.sx
            gung.y = self.boundary[1] - self.sy
            self.gungs_fired.append(gung)
        print("궁극기 사용")
    ...
    # 궁극기 리스트를 반환
    def get_gung_fired(self):
        return self.gungs_fired
